...
OUTDIR = Path("assets/figures/detr0628")
COMBINED_DRAWIO_PATH = OUTDIR / "detr0628_variants.drawio"
WIDTH = 1960
HEIGHT = 980
...
VARIANTS = [
    Variant0628(
        slug="b6-plain-hfscc",
        title="DETR0628 B6 Plain HFSCC",
        run_name="runs/detect/detr0628/b6-plain-hfscc",
        yaml_name="exp_cfg/detr0628/b6-plain-hfscc.yaml",
        note="HFSCC takes plain P3/P4 projections and Y5, then emits calibrated P3c/P4c/P5c before the final FPN/PAN fusion.",
        p3_mdhifi=False,
        p4_mdhifi=False,
    ...
    Variant0628(
        slug="b6-p3-hfscc",
        title="DETR0628 B6 P3 HFSCC",
        run_name="yaml only: b6-p3-hfscc",
        yaml_name="exp_cfg/detr0628/b6-p3-hfscc.yaml",
        note="Only the P3 branch uses MDHIFI before HFSCC; P4 stays plain and HFSCC keeps noise suppression enabled.",
        p3_mdhifi=True,
        p4_mdhifi=False,
        noise_suppression=True,
    ),
    Variant0628(
        slug="b6-p3p4-hfscc",
        title="DETR0628 B6 P3P4 HFSCC",
        run_name="runs/detect/detr0628/b6-p3p4-hfscc",
        yaml_name="exp_cfg/detr0628/b6-p3p4-hfscc.yaml",
        note="Both P3 and P4 are enhanced by MDHIFI before HFSCC performs cross-scale calibration and consistency compensation.",
        p3_mdhifi=True,
        p4_mdhifi=True,
        noise_suppression=True,
    ),
    Variant0628(
        slug="b6-p3p4-hfscc-nonoise",
        title="DETR0628 B6 P3P4 HFSCC NoNoise",
        run_name="yaml only: b6-p3p4-hfscc-nonoise",
        yaml_name="exp_cfg/detr0628/b6-p3p4-hfscc-nonoise.yaml",
        note="Same as the P3P4-HFSCC variant, but HFSCC noise suppression is disabled so only the consistency compensation path remains active.",
        p3_mdhifi=True,
        p4_mdhifi=True,
...
def subtitle_lines(v: Variant0628) -> list[str]:
    return [f"Run: {v.run_name}", f"YAML: {v.yaml_name}"]


def build_boxes(v: Variant0628) -> dict[str, Box]:
    hfscc_label = "HFSCC\nshift align + consistency\nnoise suppression on" if v.noise_suppression else "HFSCC\nshift align + consistency\nnoise suppression off"
    items = [
        ...
        Box("up_p5c", "Up(P5c)\n20 -> 40", 1450, 240, 145, 54, TAN),
        Box("y4", "Concat Up(P5c)+P4c\nRepC3 -> Y4 [40]", 1630, 225, 240, 78, PEACH),
        Box("y4_proj", "Y4 proj\n1x1 Conv", 1630, 340, 180, 58, TAN),
        Box("up_y4", "Up(Y4 proj)\n40 -> 80", 1450, 430, 145, 54, TAN),
        Box("p3_out", "Concat Up(Y4 proj)+P3c\nRepC3 -> P3_out [80]", 1630, 415, 250, 78, PEACH),
        Box("down_p3", "Down(P3_out)\nConv s=2\n80 -> 40", 1450, 560, 170, 70, TAN),
        Box("p4_out", "Concat Down(P3_out)+Y4 proj+P4c\nRepC3 -> P4_out [40]", 1630, 545, 280, 78, PEACH),
...
def build_edges(v: Variant0628) -> list[Edge]:
    edges = [
        Edge("input", "backbone"),
        Edge("backbone", "p3"),
        Edge("p3", "p4"),
    ...
    if v.p4_mdhifi:
        edges.extend([Edge("p4_proj", "p4m"), Edge("p4m", "hfscc", points=((795, 317), (860, 317), (860, 330), (900, 330)))])
    else:
        edges.append(Edge("p4_proj", "hfscc", points=((570, 317), (900, 317))))

    if v.p3_mdhifi:
        edges.extend([Edge("p3_proj", "p3m"), Edge("p3m", "hfscc", points=((795, 457), (860, 457), (860, 430), (900, 430)))])
    else:
...
def title_cells(v: Variant0628, page_id: str) -> list[str]:
...
def drawio_page(v: Variant0628, idx: int) -> str:
    page_id = f"detr0628_v{idx}"
    cells = [
...
def write_single_drawio(v: Variant0628, idx: int) -> Path:
    path = OUTDIR / v.drawio_name
    path.write_text(f'<mxfile host="codex" pages="1">\n{drawio_page(v, idx)}\n</mxfile>\n', encoding="utf-8")
    return path


def write_svg(v: Variant0628) -> Path:
    boxes = build_boxes(v)
    edges = build_edges(v)
    parts = [
