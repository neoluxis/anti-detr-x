...
__all__ = ['BasicBlock', 'BottleNeck', 'BasicBlock_Attention', 'BasicBlock_WTConv', 'BottleNeck_WTConv', 'MPA', 'WTConv2d', 'HIFI']
...
class BasicBlock_Attention(nn.Module):
    ...
    def __init__(self, ch_in, ch_out, stride, shortcut, act='relu', variant='d'):
        ...
        self.branch2a = ConvNormLayer(ch_in, ch_out, 3, stride, act=act)
        self.branch2b = WTConv2d(ch_out, ch_out, 3)
        self.act = nn.Identity() if act is None else get_activation(act)
        # self.attention = MPA(ch_out)
